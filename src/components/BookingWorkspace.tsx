...
import { Appointment, AppointmentStatus, Doctor, DoctorAvailability, DoctorBookingRequest, Lead, SystemUser } from '../types';

type Props = { currentUser: SystemUser; prefillLeadId?: string | null; onPrefillHandled?: () => void };
...
export default function BookingWorkspace({ currentUser, prefillLeadId, onPrefillHandled }: Props) {
  ...
  const refreshBookingData = async () => {
    ...
    setLoading(true);
    const [freshLeads, freshAppointments, freshRequests] = await Promise.all([
      ...
      DatabaseService.getDoctorBookingRequests({ doctorId: selectedDoctorId || undefined }),
    ]);
    setLeads(freshLeads);
  ...
  useEffect(() => {
    ...
    const lead = leads.find(item => item.id === prefillLeadId);
    if (!lead) return;
    setSelectedLead(lead);
    setPatientQuery(lead.name);
    onPrefillHandled?.();
  }, [prefillLeadId, leads, onPrefillHandled]);
  ...
  const handleCreateDoctorRequest = async (event: React.FormEvent) => {
    ...
    const result = await DatabaseService.createDoctorBookingRequest({
      ...
      requestedEndTime: timeValue(minutes(startTime) + duration),
      requestNote: notes.trim(),
      createdBy: currentUser.name,
      createdByRole: currentUser.role,
    }, currentUser);
    if (!result.success) { setNotice({ type: 'error', text: result.error || 'تعذر إرسال طلب الطبيب.' }); return; }
    setNotice({ type: 'success', text: 'تم إرسال طلب الحجز للطبيب. سيظهر الرد داخل ملف المريض فورًا.' });
    setSelectedLead(null); setPatientQuery(''); setNotes('');
    await refreshBookingData();
  };

  const handleConfirmRequest = async (request: DoctorBookingRequest) => {
    const result = await DatabaseService.confirmDoctorBookingRequest(request, currentUser);
    if (!result.success) { setNotice({ type: 'error', text: result.error || 'تعذر تأكيد موافقة المريض.' }); return; }
    setNotice({ type: 'success', text: 'تم تأكيد موافقة المريض وإنشاء الموعد المؤكد.' });
    await refreshBookingData();
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6" dir="ltr">
      <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
        <div>
          <div className="flex items-center gap-3"><div className="p-2.5 rounded-2xl bg-emerald-500/10 border border-emerald-500/20"><CalendarDays className="w-6 h-6 text-emerald-400" /></div><div><h1 className="text-3xl font-extrabold text-white">دورة طلب الحجز</h1><p className="text-sm text-neutral-500 mt-1">ينشئ الفريق الطلب، يقرر الطبيب، ثم يؤكد الفريق موافقة المريض.</p></div></div>
        </div>
      ...
      <div className="glass-panel rounded-2xl border border-neutral-800 p-4"><div className="flex flex-col md:flex-row md:items-center gap-3"><span className="text-xs font-bold text-neutral-400">Clinic / department</span><select value={selectedClinic} onChange={e => { const value = e.target.value as typeof selectedClinic; setSelectedClinic(value); setSelectedDepartment('All'); setSelectedDoctorId(''); }} className="w-full md:w-56 px-3 py-2 rounded-xl bg-neutral-950 border border-neutral-800 text-sm text-white"><option value="All">All organizations</option><option value="Eye World">Eye World / دنيا العيون</option><option value="Top Care">Top Care</option></select><select value={selectedDepartment} onChange={e => { setSelectedDepartment(e.target.value as typeof selectedDepartment); setSelectedDoctorId(''); }} className="w-full md:w-56 px-3 py-2 rounded-xl bg-neutral-950 border border-neutral-800 text-sm text-white"><option value="All">All departments</option>{selectedClinic !== 'Top Care' && <><option value="Dr. Ihab Clinic">Dr. Ihab Clinic / عيادة د. إيهاب</option><option value="Eye World Hospital">Eye World Hospital / مستشفى دنيا العيون</option></>}{selectedClinic !== 'Eye World' && <><option value="Dermatology">Dermatology / الجلدية</option><option value="Dentistry">Dentistry / الأسنان</option></>}</select><span className="text-[11px] text-neutral-600">{clinicDoctors.length} doctors configured</span></div></div>\n\n      <div className="grid grid-cols-1 xl:grid-cols-[1.15fr_0.85fr] gap-6">
        <section className="space-y-6">
          <div className="glass-panel rounded-2xl border border-neutral-800 p-5 space-y-5">
            <div className="flex items-center justify-between"><div><h2 className="text-base font-extrabold text-white flex items-center gap-2"><UserRound className="w-4 h-4 text-emerald-400" /> طلب للطبيب</h2><p className="text-xs text-neutral-500 mt-1">ابحث عن ملف مريض موجود ثم أرسل للطبيب الموعد المقترح.</p></div><span className="text-[10px] uppercase tracking-widest font-bold text-neutral-600">{currentUser.role}</span></div>
            ...
            <form onSubmit={handleCreateDoctorRequest} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <label className="md:col-span-2 text-xs font-bold text-neutral-400">ملاحظة للفريق والطبيب<input value={notes} onChange={e => setNotes(e.target.value)} placeholder="سبب الزيارة أو أي سياق مهم للطبيب" className="mt-1 w-full px-3 py-2.5 rounded-xl bg-neutral-950 border border-neutral-800 text-white" /></label>
              <button type="submit" className="md:col-span-2 flex items-center justify-center gap-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white py-3 text-sm font-extrabold"><CheckCircle2 className="w-4 h-4" /> إرسال طلب للطبيب</button>
            </form>
          </div>

          <div className="glass-panel rounded-2xl border border-neutral-800 overflow-hidden">
            <div className="px-5 py-4 border-b border-neutral-800 flex items-center justify-between"><div><h2 className="text-base font-extrabold text-white">ردود الطبيب</h2><p className="text-xs text-neutral-500">كل رد يرجع تلقائيًا لملف المريض ولصاحب الطلب.</p></div><span className="text-xs font-bold text-emerald-400">{requests.length} طلب</span></div>
            <div className="divide-y divide-neutral-800">{requests.length === 0 ? <div className="p-8 text-center text-neutral-600 text-sm">لا توجد طلبات للطبيب المحدد.</div> : requests.slice(0, 8).map(request => <div key={request.id} className="p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3"><div><div className="flex items-center gap-2"><span className="text-xs font-extrabold text-white">{request.patientName}</span><span className={`text-[10px] px-2 py-1 rounded-full border ${request.requestStatus === 'Approved' ? 'text-emerald-300 border-emerald-500/20 bg-emerald-500/10' : request.requestStatus === 'Declined' ? 'text-rose-300 border-rose-500/20 bg-rose-500/10' : 'text-amber-300 border-amber-500/20 bg-amber-500/10'}`}>{request.requestStatus}</span></div><p className="text-[11px] text-neutral-400 mt-1">{request.doctorName} · {request.requestedDate} · {request.requestedStartTime}</p>{request.doctorResponseNote && <p className="text-xs text-neutral-300 mt-1">رد الطبيب: {request.doctorResponseNote}</p>}</div>{request.requestStatus === 'Approved' && !isDoctor && <button onClick={() => handleConfirmRequest(request)} className="px-3 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-[10px] font-bold">تأكيد موافقة المريض</button>}</div>)}</div>
          </div>
